# Feature Specification: Specificity Platform — Core Project Foundation
...
A developer runs a single command (or follows a short AI-guided script) inside an empty or pre-existing project folder. The command detects what is already installed, installs or links the required third-party tools (Graphify, Obsidian, SpecKit/Specify) according to their install mode (local vs global prerequisite), and then installs the Specificity custom skills and workflow guides locally. After the command completes the developer can immediately begin using the full AI-assisted workflow.
...
**Why this priority**: This is the primary token-efficiency mechanism — the core differentiator of Specificity over vanilla SpecKit usage.
...
**Independent Test**: Run a full speckit feature lifecycle on a test project with Specificity active; compare spec detail, plan accuracy, and implementation correctness against a baseline vanilla SpecKit run on the same feature.
...
1. **Given** any Specificity workflow step, **When** Caveman mode is active, **Then** all AI responses are compressed using the Caveman skill conventions.
...
A developer updates one third-party tool (e.g., a new version of SpecKit/Specify is released globally) without needing to re-initialise the entire Specificity stack. The modular structure allows each component (Graphify, Obsidian, SpecKit, Specificity custom skills) to be updated independently.

**Why this priority**: Ensures long-term maintainability and reduces friction of keeping the stack current.

**Independent Test**: Update the global SpecKit installation; verify Specificity custom skills still function correctly and no configuration files need manual merging.

**Acceptance Scenarios**:

1. **Given** SpecKit is updated globally, **When** a developer runs the Specificity update check, **Then** the custom skills layer adapts to the new SpecKit version without manual intervention.
...
- **FR-002**: The initialisation mechanism MUST install Specificity's custom skills and workflow documents locally within the project, in a structure consistent with how Specify installs SpecKit.
...
- **FR-004**: Specificity MUST install standard SpecKit globally (or guide the user to do so) so that upstream SpecKit updates can be applied without modifying Specificity's custom layer.
...
- **FR-007**: All Specificity-extended SpecKit skills MUST decorate (not replace) the vanilla SpecKit commands, following a decorator pattern.
...
- **FR-009**: The Caveman skill MUST be integrated into the Specificity workflow and invokable at any point to compress AI context.
- **FR-010**: Each component (Graphify, Obsidian, SpecKit, Specificity custom layer) MUST be independently updatable without requiring full re-initialisation.
...
- **SC-001**: A developer with no prior knowledge of the tool stack can complete tool installation in under 10 minutes and complete first-time initialisation (including Specificity skill deployment and first `/speckit.specify` invocation) in under 30 minutes total.
...
- GitHub Copilot reads skills from `.github/agents/` and Claude Code reads skills from `.claude/commands/` — both conventions are supported by SpecKit/Specify and used by Specificity.
...
- The Caveman skill is already installed in the developer's global agent skills directory and does not need to be installed by Specificity.
...
- Automatic conflict resolution between SpecKit upstream changes and Specificity custom skills.

...
0. **Caveman activation check**: Ensure Caveman compression is active. If not active in this session, run `/caveman full` to enable ~75% token reduction (valuable for dependency mapping and graph analysis).

0.5. **Validation**: Require explicit topic parameter. If topic not provided, halt with error and prompt user to provide feature area or topic name. Validate lat.md code index exists in `.spek/lat.md/code/` before querying. Validate lat.md symlink at `./lat.md` points to `.spek/lat.md/` (required for `lat mcp` server). Pre-check: load vault decisions to flag any blockers already documented.

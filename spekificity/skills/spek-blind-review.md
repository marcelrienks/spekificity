...
0. **Pre-check**: Validate test runner available (e.g., `pytest`, `jest`, etc. — check in PATH). Validate linter installed (e.g., `pylint`, `flake8`, `eslint` — check in PATH). Validate linter config file exists (e.g., `.pylintrc`, `.flake8`, `.eslintrc`). If any tool missing, halt with error. Run test suite. If any tests fail, report failures and stop (do not proceed to linting). Exit with error status. Capture test output to `.spek/memory/last-test-output.log`. **Token efficiency note**: Blind-review report can be verbose with many findings. If Caveman compression active, review will be presented in compressed form (~75% token reduction); if not active, consider `/caveman full` for this skill's output.

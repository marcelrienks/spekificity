...
def write_git_hook(project_path: Path, skip: bool = False) -> None:
    """Write post-commit hook. Always replaces. Skipped if skip=True, --no-git-hooks used, or .spek/.disable-git-hooks exists."""
    ...
    print_status("OK", f"post-commit hook {action} to {hook_path}")

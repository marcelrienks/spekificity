...
source: "https://medium.com/@abdulgafoorabid/how-i-cut-claude-code-token-usage-by-90-with-4-tools-custom-hooks-and-enforcement-d3f8d2488cd6"
author:
  - "[[Abid Abdul Gafoor]]"
...
- **Per-stack rule files** in `~/.claude/rules/<stack>.md`: skill gate + numbered self-check. Repo ships an empty `rules/` with `README.md`. Drop in your own. Mine stay private since they only match my stacks; examples are further down.
...
**Companion repo:** `github.com/sgaabdu4/claude-code-tips` contains every config, hook, and script below.
...
Full file: `CLAUDE.md.example`. Sections it covers:
...
**The repo ships** `**rules/**` **empty.** Just `rules/README.md` as a template. Listing my flutter/react/appwrite rules in the install would either pollute your context with stuff you don't ship, or imply they apply when they don't. The three I run (shown below as article examples, not shipped as files):
...
Point `statusLine.command` at `statusline/statusline-command.sh`.

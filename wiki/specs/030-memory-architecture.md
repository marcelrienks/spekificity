...
- **Use Cases:**
- **Document Discovery:** Agent queries "what docs are related to [topic]?"
- **Wikilink Navigation:** Follow [[reference]] chains to find related knowledge
- **Validation:** Identify broken wikilinks before committing vault
...
- First `/spek.context` call → Read all vault files
- Store hashes in vault/session/context-cache.json
- On next `/spek.context` call → Check current hashes
- If hashes match → Reuse previous read (skip Phase 1)
- If hashes differ → Re-read changed files
...
- First `/spek.context` call → Summarize, store result
- Compute cache key: `hash(vault_content + graph_content)`
- On next `/spek.context` call → Check if key matches
- If matches → Reuse previous summary (skip Phase 3, save ~3.5K tokens)
- If differs → Re-summarize
...
- ## Next Steps
- Run `/spek.plan [feature description]`

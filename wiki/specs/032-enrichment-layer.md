...
- Markdown is well-formed (parseable)
- Required sections present (requirements, acceptance criteria)
- Spec doesn't contradict recent decisions (warn if it does)
- **Update Memory:**
...
- Read spec.md (identify affected modules)
- Query wiki/vault/graph/nodes.jsonl for affected modules
...
- Plan doesn't contradict recent decisions (warn if it does)
- Plan doesn't duplicate existing code (warn if it does)
...
- See [context-layer.md](031-context-layer.md) for how context is composed and made available to enrichment phases.
...
- Load context (PRE)
- Call SpecKit (CORE) with enriched input
- Validate output (POST)
...
- [decorator-wrapper-pattern.md](011-decorator-wrapper-pattern.md) — Wrapper structure
- [memory-architecture.md](030-memory-architecture.md) — Session memory containing loaded context
- [speckit-integration-contract.md](110-speckit-integration-contract.md) — Integration contract with SpecKit

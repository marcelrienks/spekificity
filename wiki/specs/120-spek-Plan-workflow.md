...
- Collect required inputs (spec.md, plan.md, etc.)
- Validate inputs exist + are well-formed
- Inject Spekificity context (decisions, patterns, code graph)
...
- ↑ NEW
- Detection:
- spek.plan queries registry on startup
- Registry returns new workflow with 'validate' step
- spek.plan discovers 'validate' is new (not in prior runs)
- Offers user: "SpecKit v3.0 includes new 'validate' step. Run it? (yes/no)"
- If yes: Includes 'validate' in this run
- If no: Skips it (maintains backward compatibility)
- **Scenario 2: Skill renamed in SpecKit v3.0**
- Old name: /clarify
- New name: /clarify-spec (same functionality, better name)
- Registry query returns /clarify-spec (not /clarify)
- spek.plan uses new name when orchestrating
- Old hardcoded calls would break; dynamic discovery prevents this
...
- Registry query returns no /analyze skill
- spek.plan skips it (not hardcoded to require it)
- Workflow adapts automatically; no code change needed
...
- ... (more steps)
- ## Error Handling
- All errors are handled per [error-handling-and-recovery.md](010-error-handling-and-recovery.md):
...
- ## Related Specs
- [speckit-integration-contract.md](110-speckit-integration-contract.md) — Integration pattern + responsibility division
- [cli-orchestration.md](121-cli-orchestration.md) — Lower-level CLI commands
- [error-handling-and-recovery.md](010-error-handling-and-recovery.md) — Cross-cutting error strategy
- [context-layer.md](031-context-layer.md) — Context injection mechanism
...
- **Dynamic discovery** (not hardcoded skill names)
- **Graceful degradation** (skip missing skills)
- **User prompts at natural points** (not overload)
...
Based on [speckit-integration-contract.md](110-speckit-integration-contract.md), the canonical workflow is:
...
See [speckit-integration-contract.md](110-speckit-integration-contract.md) for input/output contracts per skill.

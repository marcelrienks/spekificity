...
- **Recovery Flow:**
  1. Attempt to use last known good graph (cached version)
  2. If no cache: Continue with **empty graph** (no code context)
...
- **Recovery Flow:**
  1. Reduce context size: Remove least-recent lessons
  2. If still too large: Use decision index + pattern index only (compressed)
...
- **Recovery Flow:**
  1. **Retry 1:** Wait 10s, retry same command
...
- **Recovery Flow:**
  1. Detect: Monitor token usage + session memory size
  2. At 80% usage: Warn user, offer compression
...
**Code Graph:**
- [Code and Document Maps](code-and-document-maps.md)
- [Graph Refresh Strategy](graph-refresh-strategy.md)

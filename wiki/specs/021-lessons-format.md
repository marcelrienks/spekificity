...
**What Worked Well:**
1. Decorator pattern made integration clean and testable
2. Splitting specs into atomic concerns enabled parallel work
3. Vault-first approach kept Obsidian as UX tool (didn't require plugin rewrites)

**What to Do Differently:**
1. Graph merge took longer than estimated — next time allocate 2x time for validation + edge cases
2. Link discovery heuristics were fragile — consider manual linking for critical docs
3. Caveman mode slowed output iteration — use full mode for drafting, compress at end

**Edge Cases to Watch:**
1. Heading ID conversion edge cases (special chars, multiple words with hyphens)
2. Symlinks in code paths (may cause duplicates or conflicts)
3. Multi-language files (TypeScript + Python mixed in single file)

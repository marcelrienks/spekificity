...
**Original ID:** C.3.8  
**See:** [Validation Patterns Archive](142-validation-patterns-archive.md#section-1-anti-sycophancy-validation-rules)
...
- Rule enforcement in `/spek.plan` phases and `/spek.implement`
...
- Extract architectural choice
- Check wiki/vault/decisions.md for related decisions
...
- Get similar past features from wiki/vault/lessons
- Calculate average LOC + patterns
- If current estimate > (avg * 1.5):
- a. Alert user
...
- Count pattern usage across recent features
- Identify consensus patterns for domain
- If spec deviates:
...
- Extract tech stack from wiki/vault/decisions
- Scan spec for new technologies
- If new tech found:
...
- Compare original spec vs current tasks
- Identify scope additions
- Alert: "Added [N] items beyond spec; confirm intended"
- ## Validation Points
- ### During `/spek.plan` specify phase
- /spek.plan (specify phase with anti-sycophancy):
- After spec generated:
- Run Rule 1: Check for vault contradictions
- Run Rule 2: Check complexity vs similar features
- Run Rule 3: Check pattern suggestions
- Run Rule 4: Check tech stack alignment
- Alerts found? → Show to user
- User can: Accept / Modify / Override with justification
- ### During `/spek.plan` plan phase
- /spek.plan (plan phase with anti-sycophancy):
- After plan generated:
- Run Rule 1: Check architecture vs decisions
- Run Rule 3: Check pattern alignment
- Run Rule 2: Check scope complexity
- User can: Accept / Re-plan / Override with justification
- ### During /spek.implement
- /spek.implement (with anti-sycophancy):
- Mid-implementation:
- Run Rule 5: Monitor for scope creep
- Run Rule 1: Check tech choices vs stack
...
- ## Spec Generation Rules (`/spek.plan` specify phase)
...
- ## Plan Generation Rules (`/spek.plan` plan phase)
...
- Add to feature session log
- Flag for RARV reflection later
- Proceed with spec
- Alert post-mortem: "Review override at feature end"
- ### Tracking Overrides
- Store in `vault/session/`:
- ## Validation Rule Overrides
- **Rule:** Complexity (1.5x higher than auth-refactor)
...
- ## Success Criteria
- ✅ Rules 1-5 implemented in `/spek.plan` phases and `/spek.implement`

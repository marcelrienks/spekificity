...
- **Process:**
- Read vault (decisions, patterns, recent lessons)
- Read repo memory (compressed context)
- Query code graph (vault/graph/nodes.jsonl)
- Summarize in caveman format
- Write to vault/session/
- **Output:**
...
- **Steps:**
- Verify git state (clean working dir, feature branch)
- Activate caveman mode (lite)
- Load vault context via `/spek.context`
- Verify code graph freshness (last sync < 1hr?)
- Report ready state
...
- Activate caveman mode (configurable: lite, full, or ultra)
- Generate lessons
...
- Incremental code graph sync (`/spek.map`)
- Sync recent items to repo memory
...
- Log error
- Suggest `/speckit.clarify` to refine inputs
- Retry specify phase inside `/spek.plan` with clarifications
- **If plan generation fails:**
- Check if spec is too vague → suggest clarify
- Check if architecture is infeasible → suggest spec revision
- Retry plan phase inside `/spek.plan` with adjustments
- **If implementation fails:**
- Log failed task(s)
- Continue with remaining tasks (partial completion valid)
- Report summary of successes/failures
- Proceed to `/spek.conclude` anyway (lessons even from partial work)
- **If vault update fails:**
- Archive artifacts for manual inspection
- Report blockers and recovery steps
- ### Fallback Patterns
...
- **Precedence:**
- `.specify/config.yaml` (highest priority)
- SpecKit defaults
- Copilot defaults (lowest)
...
- `.spek/config.yaml` (highest priority)
- Spekificity defaults
- Vault settings (if defined)
...
- `vault/graph/config.json` (highest)
- Vault defaults
- System defaults
- ## Success Criteria
...
- ✅ `/spek.context` loads and summarizes context (caveman format, 3-5K tokens)
- ✅ `/spek.prepare` verifies git state and graph freshness
- ✅ `/spek.plan` specify phase injects context, calls speckit, validates output
- ✅ `/spek.plan` plan phase injects context + code graph, calls speckit, validates output
- ✅ `/speckit.tasks` generates tasks without spekificity wrapper
- ✅ `/spek.implement` calls speckit, collects artifacts, updates memory
- ✅ `/spek.conclude` generates lessons, updates vault, syncs to repo memory
- ✅ `/spek.plan` orchestrates entire flow sequentially
- ✅ Error handling works: retry, fallback, partial completion, recovery
- ## References
- **Related specs:**
- [Spekificity Workflow](../workflow.md)
- [extracted spec Code and Document Maps](056-code-and-document-maps.md)
- [extracted spec Memory Architecture](030-memory-architecture.md)
...
- [/spek.context spec](031-context-layer.md)
- [/spek.prepare spec](100-prepare-command.md)
- [/spek.conclude spec](102-conclude-command.md)

...
**Integration:** The vault is Git-backed and version-controlled. The Obsidian desktop app is optional for visualization; however, the Obsidian CLI is required for Spekificity's automated vault operations (syncs, exports, and scripted metadata extraction). The vault also works equally well in any markdown editor or command-line tools for manual editing.
